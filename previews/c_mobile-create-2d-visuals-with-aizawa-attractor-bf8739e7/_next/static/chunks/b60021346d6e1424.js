...
`,includeInSlideshow:!1},aizawaGhostVeil:{name:"Aizawa Ghost Veil",description:"Ethereal enclosed Aizawa shells with breathing, creature-like pulses",sim:`
in vec2 vUv;
out vec4 fragColor;
...
void main() {
    ...
    vec3 ghost = mix(vec3(0.03, 0.05, 0.08), vec3(0.56, 0.95, 1.0), smoothstep(0.04, 0.5, envelope));
    ghost += vec3(0.24, 0.46, 0.9) * smoothstep(0.01, 0.09, filament);

    vec3 prev = texture(uPrevState, vUv + vec2(0.0008, -0.0012)).rgb * (0.968 - 0.08 * uDt);
    vec3 col = max(prev, ghost * (0.66 + 0.95 * envelope));
...
void main() {
    ...
    float pulse = 0.96 + 0.06 * sin(uTime * 1.7 + vUv.y * 7.0);
    c *= pulse;

    float halo = smoothstep(0.9, 0.2, distance(vUv, vec2(0.5)));
    c += vec3(0.045, 0.06, 0.11) * halo;

    fragColor = vec4(clamp(c, 0.0, 1.0), uOpacity);
}
`},aizawaSpectralBloom:{name:"Aizawa Spectral Bloom",description:"Spectral petal fields formed by animated enclosed Aizawa trajectories",sim:`
in vec2 vUv;
out vec4 fragColor;
