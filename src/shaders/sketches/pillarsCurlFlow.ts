/**
 * PILLARS OF CREATION - CURL FLOW SMOKE
 */
...
float densityField(vec3 p, float t) {
    vec3 flow = vec3(
        fbm3(p * 0.85 + vec3(0.0, t * 0.05, 0.0)),
        fbm3(p.yzx * 0.85 + vec3(4.0, 0.0, t * 0.04)),
        fbm3(p.zxy * 0.85 + vec3(0.0, 2.2, -t * 0.03))
    ) - 0.5;

    vec3 advected = p + 0.75 * flow;
    advected.y *= 0.72;

    return clamp(
        fbm3(advected * 0.9)
        - 0.3 * fbm3(advected * 2.0)
        + 0.25 * smoothstep(-0.2, 1.6, advected.y),
        0.0,
        1.0
    );
}
...
void main() {
    ...
    float tr = 1.0;
    float ray = 0.0;
    const float maxSteps = 84.0;
    const float stepSize = 0.065;
