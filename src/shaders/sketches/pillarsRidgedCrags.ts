/**
 * PILLARS OF CREATION - RIDGED CRAGS
 */
...
float densityField(vec3 p) {
    vec3 scaled = vec3(p.x * 0.65, p.y * 0.14, p.z * 0.65);
    float ridge = 1.0 - abs(2.0 * fbm3(scaled) - 1.0);
    float density = smoothstep(0.35, 0.95, ridge) * smoothstep(-0.2, 1.8, p.y);
    float dust = fbm3(scaled * 2.8);
    return max(0.0, density - 0.62 * dust);
}
